...
bool notMarked(bool *visited,int i,int j){
    for(int k=i;k<j;k++){
        if(visited[k]){
            return false ;
        }
...
int main() {
    ...
    for(int i=1;i<=s.size();i++){
        for(int j=0 ; j <= s.size()-i ; j++){
            ...
            ll x = stoll(sub) ;
           // cout << "i="<<i<<"      j="<<j<<"      x="<<x <<"       "<< boolalpha<< isCBNumber(x) << "      "<< boolalpha <<notMarked(visited,j,i+j) <<endl;
            if(isCBNumber(x) && notMarked(visited,j,i+j)){
                count++;
                //cout << "CB" << endl;
                for(int k=j;k<i+j;k++){
                    visited[k] = true;
